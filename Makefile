...
# Immediately check if all required variables are defined.
If not, stop and show an error.
...
ifneq ($(strip $(FIRST_GOAL)),)
  ifneq ($(filter $(FIRST_GOAL), $(KNOWN_TARGETS)), $(FIRST_GOAL))
    $(error Unknown target: '$(FIRST_GOAL)'.
Run 'make help' for a list of valid targets.)
  endif
endif
...
check-env-vars:
	@if [ -z "$(FRONTEND_DOMAIN)" 
] || [ -z "$(BACKEND_DOMAIN)" ] || [ -z "$(GITHUB_REPO)" ];
then \
...
\
	...
	@if [ "$${CLOUD_SHELL}" != "true" ] && [ ! -f "$(ADC_FILE)" ];
then \
...
update-tfvars:
	...
	@node -e 'try { console.log(JSON.stringify(JSON.parse(require("fs").readFileSync("server/src/config/pass-templates.json", "utf-8")))) } catch (e) { console.error("Error: Could not read or parse server/src/config/pass-templates.json. Make sure it exists and is valid JSON.");
process.exit(1); }' >> $(TFFILE).tmp
	@echo 'EOT' >> $(TFFILE).tmp
	@echo 'pass_google_config = <<EOT' >> $(TFFILE).tmp
	@node -e 'try { console.log(JSON.stringify(JSON.parse(require("fs").readFileSync("server/src/config/google-wallet-templates.json", "utf-8")))) } catch (e) { console.error("Error: Could not read or parse server/src/config/google-wallet-templates.json. Make sure it exists and is valid JSON.");
process.exit(1); }' >> $(TFFILE).tmp
	@echo 'EOT' >> $(TFFILE).tmp
...
create-service-account: check-gcp-project
	...
	@if gcloud iam service-accounts describe $(SERVICE_ACCOUNT_EMAIL) --project=$(PROJECT_ID) > /dev/null 2>&1;
then \
...
create-workload-identity: check-gcp-project
	...
	@if gcloud iam workload-identity-pools describe $(WORKLOAD_POOL_ID) --location="global" --project=$(PROJECT_ID) > /dev/null 2>&1;
then \
...
\
	...
	@if gcloud iam workload-identity-pools providers describe $(WORKLOAD_PROVIDER_ID) --workload-identity-pool=$(WORKLOAD_POOL_ID) --location="global" --project=$(PROJECT_ID) > /dev/null 2>&1;
then \
...
create-secrets: check-gcp-project
	...
	@for secret in $(SECRET_KEY) $(SECRET_CERT) $(SECRET_WWDR) $(SECRET_GOOGLE_WALLET_SA_KEY);
do \
...
add-secrets-placeholder: create-secrets
	...
	@if [ -z "$$(gcloud secrets versions list $(SECRET_KEY) --limit=1 --format='value(name)' --project=$(PROJECT_ID))" ];
then \
...
add-private-key:
	...
	@if [ -f server/certs/$(SIGNER_KEY_FILE) ];
then \
...
add-placeholder-certificate: add-private-key
	...
	@if [ -f server/certs/$(SIGNER_CERT_FILE) ];
then \
...
then \
	...
	if [ ! -f "$$arg" ];
then echo "❌ Error: File not found at '$$arg'"; exit 1; fi; \
...
add-local-https-certs:
	...
	@if [ -f server/certs/$(LOCALHOST_KEY_FILE) ] && [ -f server/certs/$(LOCALHOST_CERT_FILE) ];
then \
...
create-state-bucket: check-gcp-project
	...
	@if gsutil ls gs://$(PROJECT_ID)-tfstate > /dev/null 2>&1;
then \
